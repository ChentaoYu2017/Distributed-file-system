...
public final class BlockStore {
    ...
    static class BlockStoreImpl extends BlockStoreGrpc.BlockStoreImplBase {
        ...
        @Override
        public void storeBlock(Block request, final StreamObserver<Empty> responseObserver) {
            Empty response = Empty.newBuilder().build();
            String hash = request.getHash();
            byte[] data = request.getData().toByteArray();
            // check and add to map
            if (!blockMap.containsKey(hash)) {
                blockMap.put(hash, data);
            }
        ...
        @Override
        public void getBlock(Block request, final StreamObserver<Block> responseObserver) {
            String hash = request.getHash();

            byte[] data = blockMap.get(hash);
            Block response = Block.newBuilder().setHash(hash).setData(ByteString.copyFrom(data)).build();            
            responseObserver.onNext(response);
            responseObserver.onCompleted();
        }

        @Override
        public void hasBlock(Block request, final StreamObserver<SimpleAnswer> responseObserver) {
